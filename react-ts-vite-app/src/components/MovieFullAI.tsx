...
function MoviesFullAI() {
    ...
    useEffect(() => {
        async function fetchMoviesCount() {
            ...
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL}/movies?type=fullAI`);
                if (!response.ok) {
                    throw new Error("Erreur lors de fetchAllfullAI");
                }
                const data = await response.json();
                setTotalMovies(data.length); // Mettre à jour le total
            } catch (err: any) {
                setError(err.message || "Erreur lors du fetchAllfullAI");
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        <div>        <div>
            ...
            <h2 className="flex items-center justify-center px-5 text-secondary md:text-2xl font-bold my-8 text-xl" onClick={toggleAllMovies}>
                ...
                <span className="grow ml-2">{t('full_ai_title', { ns: 'Galery' }) + " : " + t('total_films', { ns: 'Galery', count: totalMovies })}</span>
            </h2>
